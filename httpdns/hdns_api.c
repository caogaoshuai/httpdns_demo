...
static void empty_hdns_resv_done_callback(hdns_status_t* status, hdns_list_head_t* results, void* param)
{
	...
	{
		hdns_log_error("Pre-resolution failed: code:%d, error_code:%s, error_msg:%s, session_id:%s", status->code, status->error_code, status->error_msg, status->session_id);
	}
}
...
int hdns_sdk_init()
{
	...
	if ((s = apr_thread_pool_create(&g_hdns_api_thread_pool, HDNS_THREAD_POOL_CORE_SIZE, HDNS_THREAD_POOL_MAX_SIZE, g_hdns_api_pool)) != APR_SUCCESS)
	{
...
hdns_status_t hdns_client_start(hdns_client_t* client)
{
	...
	// 异步预解析
	status = hdns_get_results_for_hosts_async_with_cache(client, client->config->pre_resolve_hosts, HDNS_QUERY_AUTO, NULL, empty_hdns_resv_done_callback, NULL);
	if (!hdns_status_is_ok(&status))
	{
		return status;
	}
...
void hdns_client_set_schedule_center_region(hdns_client_t* client, const char* region)
{
	...
	if (changed)
	{
		...
		hdns_list_dup(scheduler->ipv4_resolvers, hdns_config_get_boot_servers(client->config, true), hdns_to_list_clone_fn_t(apr_pstrdup));
		hdns_list_dup(scheduler->ipv6_resolvers, hdns_config_get_boot_servers(client->config, false), hdns_to_list_clone_fn_t(apr_pstrdup));
	}
...
void hdns_client_enable_update_cache_after_net_change(hdns_client_t* client, bool enable)
{
	if (enable)
	{
		hdns_net_add_chg_cb_task(client->net_detector, HDNS_NET_CB_UPDATE_CACHE, (hdns_net_chg_cb_fn_t)hdns_update_cache_on_net_change, client, client->cache);
	}
	else
	{
		hdns_net_cancel_chg_cb_task(client->net_detector, client->cache);
	}
}
...
hdns_status_t hdns_resv_req_set_client_ip(hdns_resv_req_t* req, const char* client_ip)
{
	...
	{
		return hdns_status_error(HDNS_INVALID_ARGUMENT, HDNS_INVALID_ARGUMENT_CODE, "req is null or client_ip is blank", req->session_id);
	}
...
hdns_status_t hdns_resv_req_set_host(hdns_resv_req_t* req, const char* host)
{
	...
	{
		return hdns_status_error(HDNS_INVALID_ARGUMENT, HDNS_INVALID_ARGUMENT_CODE, "req is null or host is blank", req->session_id);
	}
	apr_thread_mutex_lock(req->lock);
...
hdns_status_t hdns_resv_req_append_sdns_param(hdns_resv_req_t* req, const char* key, const char* value)
{
	...
	{
		return hdns_status_error(HDNS_INVALID_ARGUMENT, HDNS_INVALID_ARGUMENT_CODE, "req is null or host is blank or value is blank", req->session_id);
	}
	apr_thread_mutex_lock(req->lock);
...
hdns_status_t hdns_resv_req_set_query_type(hdns_resv_req_t* req, hdns_query_type_t query_type)
{
	...
	{
		return hdns_status_error(HDNS_INVALID_ARGUMENT, HDNS_INVALID_ARGUMENT_CODE, "req is null or query_type is invalid", req->session_id);
	}
	apr_thread_mutex_lock(req->lock);
...
hdns_status_t hdns_resv_req_set_cache_key(hdns_resv_req_t* req, const char* cache_key)
{
	...
	{
		return hdns_status_error(HDNS_INVALID_ARGUMENT, HDNS_INVALID_ARGUMENT_CODE, "req is null or cache_key is blank", req->session_id);
	}
	apr_thread_mutex_lock(req->lock);
...
hdns_status_t hdns_list_add_str(hdns_list_head_t* list, const char* str)
{
	...
	return hdns_status_error(HDNS_INVALID_ARGUMENT, HDNS_INVALID_ARGUMENT_CODE, "list is invalid or str is blank", NULL);
}

hdns_status_t hdns_get_result_for_host_sync_with_custom_request(hdns_client_t* client, const hdns_resv_req_t* req, hdns_list_head_t** results)
{
	if (NULL == results)
	{
		return hdns_status_error(HDNS_INVALID_ARGUMENT, HDNS_INVALID_ARGUMENT_CODE, "The results_pt is null.", client->config->session_id);
	}
...
static hdns_status_t hdns_get_result_for_host_sync(hdns_client_t* client, const char* host, hdns_query_type_t query_type, const char* client_ip, bool using_cache, hdns_list_head_t** results)
{
	if (NULL == results)
	{
		return hdns_status_error(HDNS_INVALID_ARGUMENT, HDNS_INVALID_ARGUMENT_CODE, "The results is null.", client->config->session_id);
	}
...
hdns_status_t hdns_get_result_for_host_sync_with_cache(hdns_client_t* client, const char* host, hdns_query_type_t query_type, const char* client_ip, hdns_list_head_t** results)
{
	return hdns_get_result_for_host_sync(client, host, query_type, client_ip, true, results);
}

hdns_status_t hdns_get_result_for_host_sync_without_cache(hdns_client_t* client, const char* host, hdns_query_type_t query_type, const char* client_ip, hdns_list_head_t** results)
{
	return hdns_get_result_for_host_sync(client, host, query_type, client_ip, false, results);
}

static hdns_status_t hdns_get_results_for_hosts_sync(hdns_client_t* client, const hdns_list_head_t* hosts, hdns_query_type_t query_type, const char* client_ip, bool using_cache, hdns_list_head_t** results)
{
	if (NULL == results)
	{
		return hdns_status_error(HDNS_INVALID_ARGUMENT, HDNS_INVALID_ARGUMENT_CODE, "The results_pt is null.", client->config->session_id);
	...
	{
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "hosts is empty", client->config->session_id);
	}
	if (query_type > HDNS_QUERY_BOTH)
	{
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "query_type is invalid", client->config->session_id);
	}
...
hdns_status_t hdns_get_results_for_hosts_sync_with_cache(hdns_client_t* client, const hdns_list_head_t* hosts, hdns_query_type_t query_type, const char* client_ip, hdns_list_head_t** results)
{
	return hdns_get_results_for_hosts_sync(client, hosts, query_type, client_ip, true, results);
}

hdns_status_t hdns_get_results_for_hosts_sync_without_cache(hdns_client_t* client, const hdns_list_head_t* hosts, hdns_query_type_t query_type, const char* client_ip, hdns_list_head_t** results)
{
...
hdns_status_t hdns_get_result_for_host_async_with_custom_request(hdns_client_t* client, const hdns_resv_req_t* resv_req, hdns_resv_done_callback_pt cb, void* cb_param)
{
	if (NULL == cb)
	{
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "callback is null.", client->config->session_id);
	}

	hdns_pool_new(pool);
	hdns_single_resv_with_custom_req_param_t* task_param = hdns_palloc(pool, sizeof(hdns_single_resv_with_custom_req_param_t));
	...
	apr_status_t status = apr_thread_pool_push(g_hdns_api_thread_pool, hdns_single_resv_with_custom_req_task, task_param, 0, client);
...
static void* APR_THREAD_FUNC hdns_single_resv_task(apr_thread_t* thread, void* data)
{
	...
	hdns_list_head_t* results = NULL;
	hdns_status_t status = hdns_get_result_for_host_sync(param->client, param->host, param->query_type, param->client_ip, param->using_cache, &results);
	param->cb(&status, results, param->cb_param);
	hdns_list_free(results);
	hdns_pool_destroy(param->pool);
	return NULL;
...
hdns_status_t hdns_get_result_for_host_async_with_cache(hdns_client_t* client, const char* host, hdns_query_type_t query_type, const char* client_ip, hdns_resv_done_callback_pt cb, void* cb_param)
{
	if (NULL == cb)
	{
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "callback is null.", client->config->session_id);
...
hdns_status_t hdns_get_result_for_host_async_without_cache(hdns_client_t* client, const char* host, hdns_query_type_t query_type, const char* client_ip, hdns_resv_done_callback_pt cb, void* cb_param)
{
	if (NULL == cb)
	{
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "callback is null.", client->config->session_id);
...
static void* APR_THREAD_FUNC hdns_batch_resv_task(apr_thread_t* thread, void* data)
{
	...
	hdns_list_head_t* results = NULL;
	hdns_status_t status = hdns_get_results_for_hosts_sync(param->client, param->hosts, param->query_type, param->client_ip, param->using_cache, &results);
	param->cb(&status, results, param->cb_param);
	hdns_list_free(results);
	hdns_pool_destroy(param->pool);
	return NULL;
...
hdns_status_t hdns_get_results_for_hosts_async_with_cache(hdns_client_t* client, const hdns_list_head_t* hosts, hdns_query_type_t query_type, const char* client_ip, hdns_resv_done_callback_pt cb, void* cb_param)
{
	if (NULL == cb)
	{
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "callback is null.", client->config->session_id);
...
hdns_status_t hdns_get_results_for_hosts_async_without_cache(hdns_client_t* client, const hdns_list_head_t* hosts, hdns_query_type_t query_type, const char* client_ip, hdns_resv_done_callback_pt cb, void* cb_param)
{
	if (NULL == cb)
	{
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "callback is null.", client->config->session_id);
...
hdns_status_t hdns_log_set_log_file_path(const char* file_path)
{
	...
	char buf[256];
	apr_file_t* thefile;
	s = apr_file_open(&thefile, file_path, APR_FOPEN_WRITE | APR_FOPEN_CREATE | APR_FOPEN_APPEND | APR_FOPEN_XTHREAD, APR_UREAD | APR_UWRITE | APR_GREAD | APR_WREAD, g_hdns_api_pool);
	if (s != APR_SUCCESS)
	{
...
void hdns_client_cleanup(hdns_client_t* client)
{
	if (client != NULL)
	{
		...
		// 延迟30秒结束，等待正在执行的异步任务
		apr_thread_pool_schedule(g_hdns_api_thread_pool, hdns_client_cleanup_task, client, 30 * APR_USEC_PER_SEC, NULL);
	}
}
