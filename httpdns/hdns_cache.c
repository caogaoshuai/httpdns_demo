...
static int hdns_hash_do_get_keys_and_values_callback_fn(void* rec, const void* key, apr_ssize_t klen, const void* value)
{
	hdns_unused_var(key);
...
static int hdns_hash_do_get_keys_callback_fn(void* rec, const void* key, apr_ssize_t klen, const void* value)
{
	hdns_unused_var(key);
	hdns_unused_var(klen);
	hdns_list_head_t* list = rec;
	...
	// SNDS entry ignore
	if (hdns_str_is_not_blank(entry->cache_key) && hdns_str_is_not_blank(entry->host) && strcmp(entry->cache_key, entry->host))
	{
		return 1;
	}
