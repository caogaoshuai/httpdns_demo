...
hdns_status_t hdns_batch_fetch_resv_results(hdns_client_t* client, const hdns_list_head_t* hosts, hdns_query_type_t query_type, const char* client_ip, hdns_cache_t* cache);

static hdns_query_type_t unwrap_auto_query_type(hdns_net_detector_t* detector);

static int collect_resv_resp_in_cache_or_localdns(hdns_cache_t* cache, const char* host, const char* cache_key, bool enable_expired_ip, bool enable_failover_localdns, hdns_list_head_t* results, hdns_query_type_t query_type, hdns_rr_type_t rr_type);

hdns_status_t hdns_do_single_resolve_with_req(hdns_client_t* client, hdns_resv_req_t* resv_req, hdns_list_head_t* results);
...
hdns_status_t hdns_do_single_resolve(hdns_client_t* client, const char* host, const hdns_query_type_t query_type, const bool using_cache, const char* client_ip, hdns_list_head_t* results)
{
...
hdns_status_t hdns_do_single_resolve_with_req(hdns_client_t* client, hdns_resv_req_t* resv_req, hdns_list_head_t* results)
{
	...
	int ipv4_collect_status = collect_resv_resp_in_cache_or_localdns(cache, resv_req->host, cache_key, enable_expired_ip, enable_failover_localdns, results, resv_req->query_type, HDNS_RR_TYPE_A);
	int ipv6_collect_status = collect_resv_resp_in_cache_or_localdns(cache, resv_req->host, cache_key, enable_expired_ip, enable_failover_localdns, results, resv_req->query_type, HDNS_RR_TYPE_AAAA);
...
static hdns_status_t hdns_update_cache_on_net_change_with_type(hdns_net_chg_cb_task_t* task, hdns_rr_type_t rr_type)
{
	...
	} while (!hdns_status_is_ok(&status) && apr_time_sec(apr_time_now() - start) < 30 && !task->stop_signal);
...
hdns_status_t hdns_do_batch_resolve(hdns_client_t* client, const hdns_list_head_t* hosts, hdns_query_type_t query_type, bool using_cache, const char* client_ip, hdns_list_head_t* results)
{
	...
	if (using_cache)
	{
		...
		hdns_list_for_each_entry(host_cursor, hosts)
		{
			...
			case HDNS_QUERY_BOTH:
			{
				hdns_resv_resp_t* ipv4_resp = hdns_cache_table_get(client->cache, host_cursor->data, HDNS_RR_TYPE_A);
				hdns_resv_resp_t* ipv6_resp = hdns_cache_table_get(client->cache, host_cursor->data, HDNS_RR_TYPE_AAAA);
				bool v4Invalid = ipv4_resp == NULL || hdns_cache_entry_is_expired(ipv4_resp);
				bool v6Invalid = ipv6_resp == NULL || hdns_cache_entry_is_expired(ipv6_resp);
				if (v4Invalid && v6Invalid)
				{
		...
		status = hdns_batch_fetch_resv_results(client, ipv4_hosts, HDNS_QUERY_IPV4, client_ip, cache);
		if (!hdns_status_is_ok(&status) && !enable_failover_localdns && !enable_expired_ip)
		{
			goto cleanup;
		}
		status = hdns_batch_fetch_resv_results(client, ipv6_hosts, HDNS_QUERY_IPV6, client_ip, cache);
		if (!hdns_status_is_ok(&status) && !enable_failover_localdns && !enable_expired_ip)
		{
			goto cleanup;
		}
		status = hdns_batch_fetch_resv_results(client, ipv46_hosts, HDNS_QUERY_BOTH, client_ip, cache);
		if (!hdns_status_is_ok(&status) && !enable_failover_localdns && !enable_expired_ip)
		{
			goto cleanup;
		}
	...
	hdns_list_for_each_entry(host_cursor, hosts)
	{
		int ipv4_collect_status = collect_resv_resp_in_cache_or_localdns(cache, host_cursor->data, host_cursor->data, enable_expired_ip, enable_failover_localdns, results, query_type, HDNS_RR_TYPE_A);
		int ipv6_collect_status = collect_resv_resp_in_cache_or_localdns(cache, host_cursor->data, host_cursor->data, enable_expired_ip, enable_failover_localdns, results, query_type, HDNS_RR_TYPE_AAAA);
...
{
	apr_thread_mutex_lock(client->config->lock);
	int* ttl = apr_hash_get(client->config->custom_ttl_items, resp->host, APR_HASH_KEY_STRING);
	apr_thread_mutex_unlock(client->config->lock);
	if (ttl != NULL)
	{
		resp->ttl = (*ttl);
...
{
	apr_thread_mutex_lock(client->config->lock);
	int* port = apr_hash_get(client->config->ip_probe_items, resp->cache_key, APR_HASH_KEY_STRING);
	apr_thread_mutex_unlock(client->config->lock);
	if (port != NULL)
	{
		hdns_pool_new(pool);
		...
		hdns_net_add_speed_detect_task(client->net_detector, hdns_net_speed_cache_cb_fn, param, resp->ips, (*port), client, &(client->state));
	}
}
...
hdns_status_t hdns_fetch_resv_results(hdns_client_t* client, hdns_resv_req_t* resv_req, hdns_cache_t* cache)
{
	...
	while (retry_times >= 0)
	{
		...
		{
			hdns_pool_destroy(req_pool);
			return hdns_status_error(HDNS_RESOLVE_FAIL, HDNS_RESOLVE_FAIL_CODE, "failed to get a resolver", client->config->session_id);
		}
	 ...
	 {
            status = hdns_status_error(HDNS_RESOLVE_FAIL, HDNS_RESOLVE_FAIL_CODE, http_resp->extra_info->reason, client->config->session_id);
            // 建连失败后全部进行重试
		...
		else if (http_resp->status == HDNS_HTTP_STATUS_OK) 
		{
		 ...
		 {
			// 服务端异常响应
            status = hdns_status_error(HDNS_RESOLVE_FAIL, HDNS_RESOLVE_FAIL_CODE, apr_pstrcat(req_pool, "Http response body: ", hdns_buf_list_content(req_pool, http_resp->body), NULL), client->config->session_id);
