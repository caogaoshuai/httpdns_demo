...
hdns_status_t hdns_config_valid(hdns_config_t* config)
{
	if (NULL == config)
	{
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "config is null", NULL);
	}
	apr_thread_mutex_lock(config->lock);
	if (hdns_str_is_blank(config->account_id))
	{
		apr_thread_mutex_unlock(config->lock);
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "account_id is blank", config->session_id);
	}
	if (config->using_sign && hdns_str_is_blank(config->secret_key))
	{
		apr_thread_mutex_unlock(config->lock);
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "using sign but secret_key is blank", config->session_id);
	}
	if (hdns_str_is_blank(config->region))
	{
		apr_thread_mutex_unlock(config->lock);
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "region is blank", config->session_id);
	}
	if (hdns_str_is_blank(config->boot_server_region))
	{
		apr_thread_mutex_unlock(config->lock);
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "boot_server_region is blank", config->session_id);
	}
	if (config->timeout <= 0)
	{
		apr_thread_mutex_unlock(config->lock);
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "timeout is less than 0", config->session_id);
	}
	apr_thread_mutex_unlock(config->lock);
