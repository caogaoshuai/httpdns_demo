...
int32_t hdns_file_create_dir(const char* dir_path)
{
	...
	if (APR_STATUS_IS_ENOENT(rv))
	{
		...
		hdns_log_info((rv == APR_SUCCESS ? "Directory '%s' created successfully." : "Failed to create directory '%s'"), dir_path);
	}
...
char* hdns_file_read(const char* file_path, apr_pool_t* pool)
{
	...
	if (rv == APR_SUCCESS)
	{
		...
		while ((rv = apr_file_read_full(file, tmp_buf, sizeof(tmp_buf) - 1, &bytes_read)) == APR_SUCCESS || rv == APR_EOF)
		{
