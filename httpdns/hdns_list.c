...
void hdns_list_dup(hdns_list_head_t* dst_head, const hdns_list_head_t* src_head, hdns_list_clone_fn_t clone)
{
...
void hdns_list_shuffle(hdns_list_head_t* head)
{
	...
	for (size_t outer_loop_i = 0; outer_loop_i < 2 * list_size; outer_loop_i++)
	{
...
void hdns_list_sort(hdns_list_head_t* head, hdns_list_cmp_pt cmp)
{
	...
	for (cursor = hdns_list_first(head), cursor_next = cursor->next; cursor != head; cursor = cursor_next, cursor_next = cursor_next->next)
	{
		// 寻找前驱
