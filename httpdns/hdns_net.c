	...
	while (!detector->change_detector->stop_signal)
	{
		if (hdns_net_is_changed(detector))
		{
			...
			hdns_list_for_each_entry_safe(cursor, detector->change_detector->cb_tasks)
			{
				...
				apr_status_t s = apr_thread_pool_push(detector->thread_pool, hdns_net_chg_cb_task_runner, task, 0, task->ownner);
				if (s != APR_SUCCESS)
				{
...
static void* APR_THREAD_FUNC hdns_net_speed_detect_task(apr_thread_t* thread, void* data)
{
	...
	while (!detector->speed_detector->stop_signal)
	{
		...
		hdns_list_for_each_entry_safe(cursor, detector->speed_detector->tasks)
		{
			...
			apr_status_t s = apr_thread_pool_push(detector->thread_pool, hdns_net_speed_detect_runner, task, 0, task->owner);
			if (s != APR_SUCCESS)
			{
				hdns_log_error("Submit task failed");
			}
...
hdns_net_detector_t* hdns_net_detector_create(apr_thread_pool_t* thread_pool)
{
	...
	hdns_net_add_chg_cb_task(detector, HDNS_NET_CB_UPDATE_NET_TYPE, (hdns_net_chg_cb_fn_t)hdns_net_type_detector_update_cache, detector, detector);
	apr_thread_pool_push(thread_pool, hdns_net_change_detect_task, detector, 0, detector);
	...
	apr_thread_pool_push(thread_pool, hdns_net_speed_detect_task, detector, 0, detector);
...
void hdns_net_type_detector_update_cache(hdns_net_chg_cb_task_t* task)
{
	...
	} while (net_stack_type == HDNS_NET_UNKNOWN && apr_time_sec(apr_time_now() - start) < 30 && !task->stop_signal);
	...
	for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next)
	{
		...
		if (family == AF_INET || family == AF_INET6)
		{
			...
			if (hdns_is_valid_ipv6(ip) || hdns_is_valid_ipv4(ip))
			{
				...
				else if (NULL == hdns_list_search(change_detector->local_ips, ip, hdns_to_list_search_fn_t(hdns_str_search)))
				{
					is_changed = true;
				}
			}
...
void hdns_net_add_chg_cb_task(hdns_net_detector_t* detector, hdns_net_chg_cb_type_t type, hdns_net_chg_cb_fn_t fn, void* param, void* owner)
{
	if (NULL == detector)
	{
		return;
...
void hdns_net_add_speed_detect_task(hdns_net_detector_t* detector, hdns_net_speed_cb_fn_t fn, void* param, hdns_list_head_t* ips, int port, void* owner, hdns_state_e* ownner_state)
{
	if (NULL == detector)
	{
		return;
