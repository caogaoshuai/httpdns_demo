...
static char* decode_html(hdns_pool_t* pool, char* src)
{
	...
	while (src_index < length)
	{
		...
		{
			if (src_index + strlen(escape_str[i]) - 1 < length && strncmp(src + src_index, escape_str[i], strlen(escape_str[i])) == 0)
			{
...
static APR_INLINE void fill_ips_and_append_resps(hdns_resv_resp_t* resv_resp, cJSON* c_json_body, const char* ips_field, hdns_list_head_t* resv_resps)
{
...
hdns_http_response_t* hdns_resv_send_req(hdns_pool_t* req_pool, hdns_resv_req_t* resv_req)
{
	...
	const char* http_api = resv_req->using_multi ? (using_sign ? HDNS_API_SIGN_RESOLVE : HDNS_API_RESOLVE) : (using_sign ? HDNS_API_SIGN_D : HDNS_API_D);
...
void hdns_parse_resv_resp(hdns_resv_req_t* resv_req, hdns_http_response_t* http_resp, hdns_pool_t* pool, hdns_list_head_t* resv_resps)
{
...
hdns_status_t hdns_resv_req_valid(const hdns_resv_req_t* req)
{
	if (NULL == req)
	{
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "req is null", NULL);
	}
	apr_thread_mutex_lock(req->lock);
	if (hdns_str_is_blank(req->account_id))
	{
		apr_thread_mutex_unlock(req->lock);
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "account_id is blank", req->session_id);
	}
	if (req->using_sign && hdns_str_is_blank(req->secret_key))
	{
		apr_thread_mutex_unlock(req->lock);
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "using sign but secret_key is blank", req->session_id);
	}
	if (hdns_str_is_blank(req->host))
	{
		apr_thread_mutex_unlock(req->lock);
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "host is blank", req->session_id);
	}
	if (req->query_type > HDNS_QUERY_BOTH)
	{
		apr_thread_mutex_unlock(req->lock);
		return hdns_status_error(HDNS_FAILED_VERIFICATION, HDNS_FAILED_VERIFICATION_CODE, "query_type is invalid", req->session_id);
	}
	apr_thread_mutex_unlock(req->lock);
...
char* hdns_resv_resp_to_str(hdns_pool_t* pool, hdns_resv_resp_t* resp)
{
	...
	return apr_pstrcat(pool,
		"hdns_resv_resp_t("
		"host=", hdns_str_is_not_blank(resp->host) ? resp->host : "", ","
		"client_ip=", hdns_str_is_not_blank(resp->client_ip) ? resp->client_ip : "", ",",
		"type=", resp->type == HDNS_RR_TYPE_A ? "A" : "AAAA", ","
		"extra=", hdns_str_is_not_blank(resp->extra) ? resp->extra : "", ","
		"cache_key=", hdns_str_is_not_blank(resp->cache_key) ? resp->cache_key : "", ",",
		"ttl=", apr_itoa(pool, resp->ttl), ",",
		"orgin_ttl=", apr_itoa(pool, resp->origin_ttl), ",",
		"ips=", hdns_buf_list_content(pool, ips), ",",
		"from_localdns=", resp->from_localdns ? "true" : "false",
		")",
		NULL);
}
