...
static void hdns_sched_do_parse_sched_resp(hdns_pool_t* req_pool, hdns_list_head_t* body_bufs, hdns_list_head_t* ips, hdns_list_head_t* ipsv6);
...
hdns_scheduler_t* hdns_scheduler_create(hdns_config_t* config, hdns_net_detector_t* detector, apr_thread_pool_t* thread_pool)
{
...
static void hdns_parse_sched_resp_body(hdns_pool_t* req_pool, hdns_list_head_t* body_bufs, hdns_scheduler_t* scheduler)
{
...
static hdns_http_request_t* create_hdns_schd_req(hdns_scheduler_t* scheduler, hdns_pool_t* req_pool)
{
	...
	if (using_sign && hdns_str_is_not_blank(secret_key))
	{
		...
		hdns_sign_t* signature = hdns_gen_sched_req_sign(req_pool, nonce, secret_key);
...
hdns_status_t hdns_scheduler_refresh_async(hdns_scheduler_t* scheduler)
{
	...
	apr_status_t status = apr_thread_pool_push(scheduler->thread_pool, hdns_sched_refresh_task, task_param, 0, scheduler);
...
hdns_status_t hdns_scheduler_refresh_resolvers(hdns_scheduler_t* scheduler)
{
	...
	for (size_t i = 0; i < boot_server_size; i++)
	{
		...
		{
			status = hdns_status_error(HDNS_RESOLVE_FAIL, HDNS_RESOLVE_FAIL_CODE, http_resp->extra_info->reason, scheduler->config->session_id);
			continue;
		}
...
int hdns_scheduler_get(hdns_scheduler_t* scheduler, char* resolver)
{
	...
	if (HDNS_IPV6_ONLY == net_stack_type)
	{
		...
		if (scheduler->cur_ipv6_resolver_index >= (int32_t)hdns_list_size(resolve_servers))
		{
			scheduler->cur_ipv6_resolver_index = 0;
		}
	...
	else
	{
		...
		if (scheduler->cur_ipv4_resolver_index >= (int32_t)hdns_list_size(resolve_servers))
		{
			scheduler->cur_ipv4_resolver_index = 0;
		}
...
void hdns_scheduler_failover(hdns_scheduler_t* scheduler, const char* server)
{
	...
	if (scheduler->cur_ipv6_resolver_index >= (int32_t)hdns_list_size(scheduler->ipv6_resolvers))
	{
		hdns_scheduler_refresh_async(scheduler);
	}
	else if (scheduler->cur_ipv4_resolver_index >= (int32_t)hdns_list_size(scheduler->ipv4_resolvers))
	{
		hdns_scheduler_refresh_async(scheduler);
	}
	apr_thread_mutex_unlock(scheduler->lock);
...
static void hdns_probe_resolvers(hdns_scheduler_t* scheduler, bool ipv4)
{
	...
	apr_thread_mutex_lock(scheduler->lock);
	hdns_list_dup(resolvers, ipv4 ? scheduler->ipv4_resolvers : scheduler->ipv6_resolvers, hdns_to_list_clone_fn_t(apr_pstrdup));
	apr_thread_mutex_unlock(scheduler->lock);
	...
	param->ipv4 = ipv4;
	hdns_net_add_speed_detect_task(scheduler->detector, hdns_net_speed_resolver_cb_fn, param, resolvers, 80, scheduler, &(scheduler->state));
}
