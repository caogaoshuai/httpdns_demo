...
int hdns_session_pool_init(hdns_pool_t* parent_pool, int flags)
{
	...
	int s;
	char buf[256];
	if ((ecode = curl_global_init(CURL_GLOBAL_ALL & ~((flags & HDNS_INIT_WINSOCK) ? 0 : CURL_GLOBAL_WIN32))) != CURLE_OK)
	{
	...
	if ((s = apr_thread_mutex_create(&g_hdns_session_stack_mutex, APR_THREAD_MUTEX_DEFAULT, g_hdns_session_pool)) != APR_SUCCESS)
	{
