...
static hdns_sign_t* hdns_generate_sign(hdns_pool_t* pool, const char* content, const char* secret, int32_t max_offset);
...
static hdns_sign_t* hdns_generate_sign(hdns_pool_t* pool, const char* content, const char* secret, int32_t max_offset)
{
	...
	signature->pool = pool;
	hdns_log_debug("hdns_sign_t(raw=%s,timestamp=%s,sign=%s)", signature->raw, signature->timestamp, signature->sign);
	return signature;
}
