...
#define hdns_str_set(str, text)		(str)->len = strlen(text); (str)->data = (char*)text
...
HDNS_API void hdns_strip_space(hdns_string_t* str);

HDNS_API void hdns_trip_space_and_cntrl(hdns_string_t* str);

HDNS_API void hdns_unquote_str(hdns_string_t* str);

HDNS_API char* hdns_pstrdup(hdns_pool_t* p, const hdns_string_t* s);

HDNS_API int hdns_ends_with(const hdns_string_t* str, const hdns_string_t* suffix);

HDNS_API bool hdns_str_search(const char* s1, const char* s2);

HDNS_API bool hdns_str_start_with(const char* str, const char* prefix);
