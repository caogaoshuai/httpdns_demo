...
static int hdns_init_curl_url(hdns_http_transport_t* t)
{
	...
	if (querystr.len == 0)
	{
		t->curl_ctx->url = apr_psprintf(t->pool, "%s%s%s", t->req->proto, host, uristr);
	}
	else
	{
		t->curl_ctx->url = apr_psprintf(t->pool, "%s%s%s%.*s", t->req->proto, host, uristr, hdns_to_int(querystr.len), querystr.data);
	}
